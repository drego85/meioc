...
# Credits to https://github.com/JoshData/convert-outlook-msg-file.
...
def load(filename_or_stream):
  with compoundfiles.CompoundFileReader(filename_or_stream) as doc:
    ...
    return __load_message_stream(doc.root, True, doc)

def __load_message_stream(entry, is_top_level, doc):
  """ Load stream data """
  props = __parse_properties(entry['__properties_version1.0'], is_top_level, entry, doc)
  ...
  if 'BODY' in props:
    ...
    has_body = True
  # PR:30 from https://github.com/JoshData/convert-outlook-msg-file/pull/30/files
  ...
  for stream in entry:
    if stream.name.startswith("__attach_version1.0_#"):
      try:
        __process_attachment(msg, stream, doc)
...
def __process_attachment(msg, entry, doc):
  """ Load attachment stream """
  props = __parse_properties(entry['__properties_version1.0'], False, entry, doc)
...
def __parse_properties(properties, is_top_level, container, doc):
  """
...
class EMBEDDED_MESSAGE(object):
  ...
  def load(entry, doc, **kwargs):
    return __load_message_stream(entry, False, doc)
...
property_tags = {
  ...
  0x1013: ('HTML_BODY', 'BINARY'),  # PR:30 from https://github.com/JoshData/convert-outlook-msg-file/pull/30/files
